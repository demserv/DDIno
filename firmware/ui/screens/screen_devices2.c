...
static const char *TAG = "screen_devices2";
...
static void screen_init_devices2(lv_obj_t *parent)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        lv_obj_set_size(plug_state_btns[i], 50, 28);
        lv_obj_align(plug_state_btns[i], LV_ALIGN_LEFT_MID, 55, 0);
        ...
        lv_obj_align(plug_current_labels[i], LV_ALIGN_LEFT_MID, 130, 0);
    }

    ato_status_label = lv_label_create(parent);
    lv_label_set_text(ato_status_label, "ATO: --");
    lv_obj_align(ato_status_label, LV_ALIGN_BOTTOM_LEFT, 10, -30);
...
static void screen_update_devices2(void)
{
    for (int i = 0; i < 5; i++) {
        char buf[32];
        float cur = 0;
        acs712_read_plug((uint8_t)(i + 6), &cur);
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "ATO: %s", g_gs.ato_ok ? "Ativo" : "Inativo");
    lv_label_set_text(ato_status_label, buf);
}
