...
/* @requirement ALM-061 (SRS §49) Integridade/configuração crítica inválida. */
static void config_raise_integrity_alm(void)
{
    alert_manager_raise(ALM_061, true, (uint64_t)(esp_timer_get_time() / 1000000LL));
}
...
static cJSON *root_to_json(const config_root_t *root)
{
    ...
    cJSON_AddBoolToObject(ato, "digital_mode", root->ato.digital_mode);
    ...
    cJSON_AddNumberToObject(sys, "carousel_interval_s", (double)root->system.carousel_interval_s);
    cJSON_AddNumberToObject(sys, "carousel_pause_s", (double)root->system.carousel_pause_s);
    ...
    cJSON_AddBoolToObject(ph, "enabled", root->ph.enabled);
    cJSON_AddNumberToObject(ph, "warn_low_ph", root->ph.warn_low_ph);
    cJSON_AddNumberToObject(ph, "warn_high_ph", root->ph.warn_high_ph);
    cJSON_AddNumberToObject(ph, "calib_min_ph", root->ph.calib_min_ph);
    cJSON_AddNumberToObject(ph, "calib_max_ph", root->ph.calib_max_ph);
    cJSON_AddItemToObject(o, "ph", ph);
    return o;
}
...
static bool json_to_root(cJSON *json, config_root_t *root, const config_root_t *base, uint32_t *expected_crc)
{
    ...
    *expected_crc = (uint32_t)crc->valuedouble;
    ...
    if (cJSON_IsObject(ato)) {
        ...
        cJSON *dm = cJSON_GetObjectItem(ato, "digital_mode");
        if (cJSON_IsBool(dm)) root->ato.digital_mode = cJSON_IsTrue(dm);
    ...
    if (cJSON_IsObject(sys)) {
        ...
        v = cJSON_GetObjectItem(sys, "carousel_interval_s");
        if (cJSON_IsNumber(v)) root->system.carousel_interval_s = (uint16_t)v->valuedouble;
        v = cJSON_GetObjectItem(sys, "carousel_pause_s");
        if (cJSON_IsNumber(v)) root->system.carousel_pause_s = (uint16_t)v->valuedouble;
    }
    ...
    if (cJSON_IsObject(ph)) {
        cJSON *v = cJSON_GetObjectItem(ph, "enabled");
        if (cJSON_IsBool(v)) root->ph.enabled = cJSON_IsTrue(v);
        parse_number_f(ph, "warn_low_ph", &root->ph.warn_low_ph);
        parse_number_f(ph, "warn_high_ph", &root->ph.warn_high_ph);
        parse_number_f(ph, "calib_min_ph", &root->ph.calib_min_ph);
        parse_number_f(ph, "calib_max_ph", &root->ph.calib_max_ph);
    }
...
esp_err_t config_import_from_json(cJSON *json, bool dry_run, bool *valid_out, uint32_t *crc_out)
{
    ...
    if (!json_to_root(json, &incoming, &backup, &expected_crc)) {
        ...
        /* @requirement TC-CONFIG-002/TC-IMPORT-002 ConfigRoot importado inválido → rejeição + ALM-061. */
        config_raise_integrity_alm();
    ...
    err = config_set_ph(&incoming.ph); if (err) goto rollback;
