...
static uint8_t s_relay_mismatch_count[PLUG_COUNT_TOTAL] = {0};
...
static esp_err_t plug_actuate(plug_id_t id, bool on, bool is_manual)
{
    ...
    esp_err_t err = relay_abstraction_set(rid, on);
    if (err != ESP_OK && on) {
        uint64_t now_s = esp_timer_get_time() / 1000000ULL;
        char msg[48];
        snprintf(msg, sizeof(msg), "Falha ao religar plugue P%02d", (int)id);
...
void plug_manager_reload_limits(void)
{
    ...
    ESP_LOGI(TAG, "Plug limits NVS: min_on=%lus min_off=%lus lim=%.1fA",
             (unsigned long)pl->min_on_time_s, (unsigned long)pl->min_off_time_s,
             ep ? (double)ep->per_plug_current_limit_a : 0.0);
}
...
void plug_manager_init(void)
{
    ...
    for (int i = 0; i < PLUG_COUNT_TOTAL; i++) {
        ...
        if (p->id >= PLUG_ID_P03) {
            p->max_energy_wh_day = PARAM_PLUG_DEFAULT_MAX_ENERGY_WH_DAY;
        }
    }
...
void plug_manager_tick(uint64_t now_s, system_state_t sys_state, bool feed_active)
{
    ...
    for (int i = 0; i < PLUG_COUNT_TOTAL; i++) {
        plug_model_t *p = &s_plugs[i];
        p->energy_wh_today = (float)cdn_energy_get_wh_today_plug((uint8_t)p->id);

        /* @requirement RF-PLUG-003 Proteção por sobrecorrente: desenergiza e bloqueia
         * após amostras consecutivas acima do limite (inclui pós-religamento). */
        if (!p->blocked && relay_get(p->id) && p->current_a > p->current_limit_a) {
            p->overcurrent_count++;
            if (p->overcurrent_count >= 3U) {
                plug_manager_set_blocked(p->id, true);
                electric_log_event(ELECTRIC_LOG_PLUG_OVERCURRENT, (uint8_t)p->id,
                                   p->current_a, "RF-PLUG-003");
            } else {
                plug_actuate(p->id, false, false);
            }
        } else if (!p->blocked && p->current_a <= p->current_limit_a * 0.95f) {
            p->overcurrent_count = 0;
        }
        ...
        if (p->blocked) {
            p->effective_state = PLUG_EFFECTIVE_STATE_BLOCKED;
        ...
        /* ALM-041: estado efetivo diverge do relé físico */
        {
            bool relay_on = relay_get(p->id);
            if (p->effective_state == PLUG_EFFECTIVE_STATE_ON && !relay_on && !p->blocked) {
                if (++s_relay_mismatch_count[i] >= 3) {
                    if (!alert_manager_is_active((int16_t)ALM_041)) {
                        char msg[56];
                        snprintf(msg, sizeof(msg), "Mismatch rele/carga em %s", p->name);
                        alert_manager_raise_full((int16_t)ALM_041, ALERT_SEVERITY_WARNING,
                                                 ALERT_CATEGORY_PROCESS, msg, 0.0f,
                                                 "Verificar funcionamento da carga",
                                                 (uint16_t)p->id, false, true, now_s * MS_PER_SEC);
                    }
                }
            } else {
                s_relay_mismatch_count[i] = 0;
            }
        }
        ...
        if (p->effective_state == PLUG_EFFECTIVE_STATE_OFF) {
            if (p->current_a > p->current_limit_a * 0.1f) {
                ...
                if (s_bypass_sample_count[i] >= 3 && !p->bypass_detected) {
                    ...
                    /* @requirement SRS §49 ALM-030 = "Bypass detectado em plugue".
                     * WARNING, sem ACK obrigatório, não força SAFE_OFF. */
                    alert_manager_raise_full((int16_t)ALM_030, ALERT_SEVERITY_WARNING,
                        ALERT_CATEGORY_PROCESS, msg, p->current_a,
                        "Verificar fiacao/equipamento", (uint16_t)p->id,
                        false, true, now_s * MS_PER_SEC);
                }
            } else {
...
static void sync_plug_blocked_alm(void)
{
    ...
    alert_manager_try_auto_clear(ALM_032, !any_blocked);
}

esp_err_t plug_manager_unblock(plug_id_t id)
{
    plug_model_t *p = plug_manager_get(id);
    if (!p) return ESP_ERR_INVALID_ARG;
    if (!p->blocked) return ESP_ERR_INVALID_STATE;
    if (p->blocked_by_safe_state) return ESP_ERR_NOT_ALLOWED;
    if (!maintenance_mode_is_active()) return ESP_ERR_NOT_ALLOWED;
    p->overcurrent_count = 0;
    p->blocked = false;
    sync_plug_blocked_alm();
    ESP_LOGI(TAG, "Plug %d desbloqueado manualmente (RF-PLUG-014)", (int)id);
    return ESP_OK;
}
